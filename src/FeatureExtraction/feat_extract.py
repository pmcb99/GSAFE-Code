...
def parse_args():
    ...
    parser.add_argument('--need-root', action='store_true', default=False,
...
def feature_extraction_function(input_video_list,num_segments,output_folder,use_gpu):
    opt = parse_args()
    if use_gpu==False:
        opt.gpu_id = -1
    opt.save_dir = ''
    opt.num_segments = num_segments
    opt.data_list = input_video_list
    opt.model = 'i3d_resnet50_v1_kinetics400'
    ...
    for vid, vline in enumerate(data_list):
        ...
        print(video_feat.asnumpy().shape)
        if 'Normal' in video_name:
            opt.save_dir = f'/DATA/ReworkSultani/{output_folder}/UCF/all_rgbs/'+'Normal_Videos_event/'
        else:
            opt.save_dir = f'/DATA/ReworkSultani/{output_folder}/UCF/all_rgbs/'+re.sub('\d+','',video_name.split('_')[0])
        if not os.path.exists(opt.save_dir):
            os.makedirs(opt.save_dir)
        feat_file = '%s.npy' % (video_name)
        print(f'Saving {feat_file} to {opt.save_dir}')
